...
const en: Dict = {
  ...
  'dash.card.document.desc': 'Upload a video of signing to generate a PDF document.',
...
const hi: Dict = {
  ...
  'nav.document': 'दस्तावेज़ अनुवाद',
  'nav.documents': 'सहेजे गए दस्तावेज़',
  'nav.learn': 'ISL सीखें',
...
const gu: Dict = {
  ...
  'nav.document': 'દસ્તાવેજ અનુવાદ',
  'nav.documents': 'સાચવેલા દસ્તાવેજો',
  'nav.learn': 'ISL શીખો',
...
function merge(lang: LanguageCode, base: Dict): Dict {
  return SECTIONS.reduce((acc, section) => ({ ...acc, ...(section as any)[lang] }), { ...base });
}
...
export const TRANSLATIONS: Record<LanguageCode, Dict> = {
  ...
  hi: merge('hi', hi),
  gu: merge('gu', gu),
  mr: merge('en', en), // Using English as fallback base until translated
  bn: merge('en', en),
  ta: merge('en', en),
  te: merge('en', en),
};
