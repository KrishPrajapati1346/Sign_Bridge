...
export function getProgress(
  authFetch: AuthFetch,
): Promise<{ xp: number; streakDays: number; lessons: LessonProgress[]; mastery: SignMastery[] }> {
...
export function addXp(
  authFetch: AuthFetch,
  amount: number,
): Promise<{ xp: number; streakDays: number }> {
  return requestJson(authFetch, '/api/learning/xp', {
    method: 'POST',
