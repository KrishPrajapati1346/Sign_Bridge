...
export function useSpeechToText({ lang, onFinal }: UseSpeechToTextOptions): UseSpeechToText {
  ...
  // Auto-restart if we unexpectedly stopped but should still be listening.
  // Actually, wait, use-call.ts manages the `start()` / `stop()` lifecycle.
  // We can just add an auto-restart loop here or in use-call.
  // It's easier to expose a continuous mode here.
