...
import { loadPoseLibrary, getPose, type SignPose } from '@/lib/avatar/pose-library';
...
function PracticeInner() {
  ...
  const targetPose = useMemo(() => {
    return target && library ? getPose(library, target.toUpperCase()) : null;
  }, [target, library]);

  const targetPoseRef = useRef<SignPose | null>(null);
  useEffect(() => {
    ...
    if (targetPoseRef.current) {
      for (const hand of targetPoseRef.current.hands) {
        ...
        for (const lm of hand.landmarks) {
          ctx.beginPath();
          ctx.arc(lm[0] * canvas.width, lm[1] * canvas.height, 8, 0, Math.PI * 2);
          ctx.fill();
        }
      }
    }
